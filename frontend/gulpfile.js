...
// Deploy task: dist ichidagi fayllarni backend statik papkasiga ko‘chirish
gulp.task("deploy", function () {
    if (!fs.existsSync(backendStaticPath)) {
        fs.mkdirSync(backendStaticPath, {recursive: true});
    }
    return gulp.src("dist/**/*")
        .pipe(gulp.dest(backendStaticPath));
});
...
gulp.task("browserSync", function () {
    browsersync.init({
        server: "dist/"
        // index: "pages/login-v1.html"  // Default ochiluvchi faylni o‘zgartirish
    });
});
//  [ browser reload ] end
...
gulp.task(
    "build-prod",
    gulp.series("cleandist", "build", "deploy", "sass", "build-js", "build-html", "imgmin")
);
